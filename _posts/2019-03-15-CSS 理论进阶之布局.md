...
- 实现可用的三列布局具有以下要求
    ...
    - 圣杯布局
        ...
        - 使用`padding`向内预留两栏的位置模仿圣杯的形状
        - 使用`relative`使左右处于中间页面的两端遮挡预留的位置
        - 代码实现
...
- 与圣杯布局基本类似, 但是此处使用`margin`来模仿飞翼达到圣杯布局中填充的效果便能省去设置`relative`的工作
- 中间栏内中用一个块元素来充当真正存放内容的元素, 并给其与左右两栏相同宽度的`margin`
